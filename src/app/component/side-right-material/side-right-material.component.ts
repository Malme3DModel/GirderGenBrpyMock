...
export class SideRightMaterialComponent implements OnInit {
  ...
  private convertInputToDefault(value: any, unit: string): number {
  ...
  private updateDisplayValues(): void {
    this.dataset.forEach((item, index) => {
      if (item.type === 'numeric' && item.value !== null && item.value !== undefined && item.value !== '') {
        const displayValue = this.getDisplayValue(item.value, item.unit);
        this.dataset[index].value = displayValue;
      }
    });
  }

  private getDisplayValue(value: number, unit: string): number {
    if (this.unitSystem === 'metric') {
      if (unit.includes('mm')) {
        return value / 1000;
      } else if (unit.includes('N') && !unit.includes('kN')) {
